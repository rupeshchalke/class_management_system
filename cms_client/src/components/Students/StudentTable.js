...
const StudentTable = ({ setShowModal,student_data_prop }) => {
  // console.log(student)
...
//   const [standards, setStandards] = useState([]);
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      try {
        const branchResponse = await fetch('http://localhost:9998/branch_crud/read_branch/');
...
let data = [];
  const handleSaveChanges = async (id) => {
    try {
      const response = await fetch(`http://localhost:9998/student_details_crud/read_single_student/${id}`);
